...
## _app.jsxにGAのスクリプトを書き込む
`_app.jsx`は全てのページで共通のHTMLを書くコンポーネントです。**この`_app.jsx`にGA用のscriptタグを記述します。**
...
```jsx:title=src/components/GoogleAnalytics.jsx
...
`GoogleAnalytics` コンポーネントはこれだけです。先程`_app.jsx`に書いたものを切り出しただけです。

あとは`_app.jsx`でこのコンポーネントを呼び出すだけです。
