...
![](/media/2020/6/21/2020_6_21__0.png)
...
`for (const messages of mails)`で二次元配列mailsの値をmessagesに格納します。`for (const a of b)`は、配列bの値を変数aに格納する記法です。
...
`markRead()`メソッドでメールを既読にすることができます。これで毎回Gmailの未読件数の増加を防げますね。
