...
Tailwind CSSでは基本的に命名をしないので、瑣末になりがちなModifierの命名に頭を悩ませることもなく、レビューを受けてクラス名を変更する手間もありません。エンジニアが思考停止でスタイルを当てられる恩恵は少なくないでしょう。脳のリソースはサーバーサイドやフロントエンドのクリーンな設計に回してください。
