---
title: "VercelにGatsbyJSをデプロイする時にお薦めのキャッシュ設定"
...
<div class="explain">
  ...
  <div class="explain__paragraphWrapper">
    <p class="explain__paragraphContent">GatsbyJSはブログで使われることが多いけど、採用ページなど更新頻度が少ないサイトで使われるケースも増えてきているね。</p>
  </div>
</div>
...
ビルドごとに名前が変わるファイルは1年間キャッシュします。つまり、cache-controlヘッダーに`max-age=31536000`を設定します。これは、ビルドのたびにファイル名に一意なハッシュが与えられるからです。

反対に、ファイル名が変わらないコンテンツ（`page-data.json`など）は、キャッシュしないのがGatsbyJS公式の推奨設定です。
