...
そのような要望が上がり始めたあたりで、Next.js 9.4 のリリースが発表されました。9.4 で実装された ISR の機能と GAS を組み合わせると、書籍の更新フローを PO だけで完結できると考えました。
...
<div class="explain">
  ...
  <div class="explain__paragraphWrapper">
    <p class="explain__paragraphContent">ちなみに ISR は Vercel でしか実行できないという話はデマなんだよ。現在、業務では ECS を使っており、Next.js を実行している Node コンテナで ISR の機能を使っているよ。</p>
  </div>
</div>
