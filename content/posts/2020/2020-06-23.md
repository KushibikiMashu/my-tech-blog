...
amp-imタグにwidthとheightが指定されていないとAMP対応のページと見なされません。`ampify.js`内で、amp-imgタグに画像サイズを指定していきます。
...
また、AMPはimタグをLazy-Loadingに対応させるための属性`loading`に対応していないため、こちらを削除します。
...
しかし、AMPでは<a>の中で<picture>は使えません。そこで、<picture>の中の<source>を削除し、imgタグだけを残すようにします。
...
これでAMPページに通常ページと同じCSSを適用できました。[なお、当サイトのampify.jsはGitHubで公開しています。](https://github.com/KushibikiMashu/my-tech-blog/blob/master/ampify.js)
