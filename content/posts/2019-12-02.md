...
template: "post"
draft: false
slug: "/posts/solve-atcoder-by-test-driven-development"
category: "TDD"
tags:
    - "TDD"
    - "設計"
...
**1から100まで長さ**の、**L,R,U,Dがランダムに並んだ文字列**が入力される時、以下のようにTODOリストを作ることができます。

入力された文字列について、以下のように仕様を分解することができます。

- [ ] L,R,U,Dのいずれかの文字を渡した時、YesまたはNoを出力する
  - [ ] 文字Rを渡した時、Yesを出力する
  - [ ] 文字Uを渡した時、Yesを出力する
  - [ ] 文字Dを渡した時、Yesを出力する
  - [ ] 文字Lを渡した時、Noを出力する

また、「奇数文字目がR・U・Dのいずれかであり、かつ偶数文字目がL、U、DのいずれかであればYesを出力する」という条件は、「奇数文字目の少なくとも一つがL、または偶数文字目の少なくとも一つがRであれば、Noを出力することができる」と読み替えることができます。

このため、TODOリストに、下記の項目を追加します。

- [ ] 奇数文字目の少なくとも一つがLである場合、Noを出力する
    - [ ] 文字列LRRを渡した時、Noを出力する
- [ ] 偶数文字目の少なくとも一つがRである場合、Noを出力する
    - [ ] 文字列LRRを渡した時、Noを出力する
    
早速実装していきましょう。

## 1週目 - 仮実装
まずは簡単に実装できる「Rを入力すると、Yesを出力する」という項目を選びます。

### レッド
テストを記述します。文字列Rを入力して文字列Yesを出力するため、「Sampleクラスのsolveメソッドの実引数に文字Rを渡すと文字列Yesを返す」テストを記述します。
...
### リファクタリング
リファクタリングのステップでは、「テストがグリーンである間は、プログラムの挙動が変わっていないので、内部実装をどのように書き換えても良い」というスタンスに立ちます。
...
## 2週目 - 明白な実装
次は「文字Lを渡した時、Noを出力する」という項目を選びます。
 ...
 * @test
 */
public function 文字Lを渡した時、Noを出力する()
{
    $sample = new Sample();
...
また、今の実装では、文字U、Dを渡してもYesを出力します。また、問題の条件より、「Noを出力する場合以外はYesを出力する」と読み替えることができるので、TODOの項目を2つ削除してもいいと判断します。

- [x] L,R,U,Dのいずれかの文字を渡した時、YesまたはNoを出力する
  - [x] 文字Rを渡した時、Yesを出力する
  - [ ] ~~文字Uを渡した時、Yesを出力する~~
  - [ ] ~~文字Dを渡した時、Yesを出力する~~
  - [x] 文字Lを渡した時、Noを出力する
...
## 3週目
次は「文字列UULを渡した時、Noを出力する」という項目を選びます。

### レッド
テストコードを書きます。

```php
public function 文字列UULを渡した時、Noを出力する()
{
...
```shell script
1) SampleTest::文字列UULを渡した時、Noを出力する
Failed asserting that two strings are identical.
--- Expected
+++ Actual
@@ @@
...
「奇数文字目がLならNoを出力する」という一般的な条件は、「1文字がLならNoを出力する」という条件を包含しています。

現状では後者は実装済みですが、前者は未実装です。

今回のTODOリストの項目、「文字列UULを渡すとき、Noを出力する」の文字列UULは3文字目がLであることを利用して、一般化した実装をしていきましょう。
...
- [x] L,R,U,Dのいずれかの文字を渡した時、YesまたはNoを出力する
  ...
  - [ ] ~~文字Dを渡した時、Yesを出力する~~
  - [x] 文字Lを渡した時、Noを出力する
- [x] 奇数文字目の少なくとも一つがLである場合、Noを出力する
    - [x] 文字列UULを渡した時、Noを出力する
- [ ] 偶数文字目の少なくとも一つがRである場合、Noを出力する
    - [ ] 文字列URUを渡した時、Noを出力する

ここで、1文字目、3文字目をLにしたからといって、99文字目がLの時にNoを出力するかはわからないので、「奇数文字目の少なくとも一つがLである場合、Noを出力する」ことをを満たしていると言えないのでは？と考える方もおられるかもしれません。
...
## 4週目 - 最後のTODO
いよいよ最後の項目「文字列URUを渡した時、Noを出力する」です。

### レッド
まずはテストコードを追加して、テストを実行します。

```php
public function 文字列URUを渡した時、Noを出力する()
{
...
```shell script
1) SampleTest::文字列URUを渡した時、Noを出力する
Failed asserting that two strings are identical.
--- Expected
+++ Actual
@@ @@
...
- [x] 奇数文字目の少なくとも一つがLである場合、Noを出力する
    - [x] 文字列UULを渡した時、Noを出力する
- [x] 偶数文字目の少なくとも一つがRである場合、Noを出力する
    - [x] 文字列URUを渡した時、Noを出力する
...
リリースまでのスピードは幾分落ちるかもしれないですが、本番リリース後にバグが出た時の障害対応に割く時間を考えると、長期的に見て時間は節約できると自分は考えています。
