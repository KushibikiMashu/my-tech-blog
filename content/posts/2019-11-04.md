---
title: 競技プログラミングAtCoderを快適に解くためのPHPの環境構築をする方法
...
アルゴリズムを使って問題を解くことは本番と練習でも同じです。

ただ、一番の違いは、コンテストでは制限時間があることです。

また、通常のコンテストでは問題は6問出題されるため、制限時間内に解けるだけ問題を解かなければなりません。

さらに、回答を提出するスピードが他の人より早いと、自分が得られるレートが高くなるため、自分の書いたロジックが正解なのか、間違っているのか。

間違っているなら、間違っている箇所を特定するスピードが早ければ早いほど有利です。
...
## PHPUnitの便利な使い方
### ジェネレータ関数とデータプロバイダーで複数の入力をシンプルに記述する
PHPUnitには`@dataProvider`というアノテーションがあります。

データプロバイダとしての関数の返り値を、`@dataProvider メソッド名`というアノテーションをつけたメソッドの引数として扱う機能です。
...
結果、ロジックのコードを競技プログラミングの回答として提出すればよくなるんですね。

（本来、ロジックはアプリケーションコードとして記述するので、テストクラスの中にロジックが入っていることに違和感がある方は、`/src`ディレクトリを作って`/src`配下のクラスでロジック記述し、TDDで開発することも可能です）
...
上記のサンプルのテストケースでは、`testA`メソッドに`@group`アノテーションを付与しています。
...
AtCoderでは入力・出力のサンプルとして2~3パターンが提示されるため、サンプルの数だけデータプロバイダに記述しておけば、ロジックの実装に集中することができます。
