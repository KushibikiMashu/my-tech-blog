...
tags:
    - "React"
    - "設計"
description: "useContext + useReducer は、state を使うコンポーネントの階層が深い上に、state の変更パターンが多様な場面で使うと良い"
...
- useContext + useReducer は、state を使うコンポーネントの階層が深い上に、state の変更パターンが多様な場面で使うと良い
...
下記のように Context が複数になるとラッパー地獄（Wrapper Hell）になるため、Context は適切に分割するのが良いでしょう。

```tsx
const ContextWrapperHell: React.FC = ({children}) => (
