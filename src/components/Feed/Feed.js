    ...
    {edges.map((edge) => (
      <div className={styles['feed__item']} key={edge.node.fields.slug}>
        <h2 className={styles['feed__item-title']}>
          <Link className={styles['feed__item-title-link']} to={edge.node.fields.slug} onClick={clickPostEvent(edge.node.frontmatter.title)}>{edge.node.frontmatter.title}</Link>
        </h2>
        ...
        <p className={styles['feed__item-description']}>
          ...
          <span className={styles['feed__item-readmore']}>
            <Link to={edge.node.fields.slug} onClick={clickPostEvent(edge.node.frontmatter.title)}>
              記事を読む
            </Link>
          </span>
        </p>
